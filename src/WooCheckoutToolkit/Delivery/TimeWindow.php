...
class TimeWindow
{
    ...
    public function render_time_window_field(): void
    {
        ...
        $options = ['' => __('Select a time...', 'checkout-toolkit-for-woo')];
        ...
        woocommerce_form_field(
            ...
            apply_filters('checkout_toolkit_time_window_field_args', [
                'type' => 'select',
                'label' => $settings['field_label'] ?? __('Preferred Time', 'checkout-toolkit-for-woo'),
    ...
    public function validate_time_window(): void
    {
        ...
        if (!empty($settings['required']) && empty($time_window)) {
            wc_add_notice(
                sprintf(
                    ...
                    '<strong>' . esc_html($settings['field_label'] ?? __('Preferred Time', 'checkout-toolkit-for-woo')) . '</strong>'
                ),
                'error'
            );
            return;
        ...
        if (!empty($time_window)) {
            ...
            if (!in_array($time_window, $valid_values, true)) {
                wc_add_notice(
                    __('Please select a valid time window.', 'checkout-toolkit-for-woo'),
                    'error'
                );
            }
        }
    ...
    private function get_settings(): array
    {
        $defaults = [
            'enabled' => false,
            'required' => false,
            'field_label' => __('Preferred Time', 'checkout-toolkit-for-woo'),
            'time_slots' => [
                ['value' => 'morning', 'label' => __('Morning (9am - 12pm)', 'checkout-toolkit-for-woo')],
                ['value' => 'afternoon', 'label' => __('Afternoon (12pm - 5pm)', 'checkout-toolkit-for-woo')],
                ['value' => 'evening', 'label' => __('Evening (5pm - 8pm)', 'checkout-toolkit-for-woo')],
            ],
