...
class DeliveryList extends \WP_List_Table
{
    ...
    public function get_columns(): array
    {
        return [
            ...
            'order' => __('Order', 'checkout-toolkit-for-woo'),
            'customer' => __('Customer', 'checkout-toolkit-for-woo'),
            'delivery_date' => __('Delivery Date', 'checkout-toolkit-for-woo'),
            'delivery_status' => __('Delivery Status', 'checkout-toolkit-for-woo'),
            'order_status' => __('Order Status', 'checkout-toolkit-for-woo'),
            'order_total' => __('Total', 'checkout-toolkit-for-woo'),
            'actions' => __('Actions', 'checkout-toolkit-for-woo'),
        ];
    }
    ...
    public function get_bulk_actions(): array
    {
        ...
        foreach (DeliveryStatus::get_statuses() as $key => $label) {
            $actions['set_status_' . $key] = sprintf(
                ...
                __('Set to %s', 'checkout-toolkit-for-woo'),
                $label
            );
        }
        ...
        add_action('admin_notices', function () use ($updated) {
            printf(
                ...
                sprintf(
                    ...
                    esc_html__('%d delivery status(es) updated.', 'checkout-toolkit-for-woo'),
                    (int) $updated
                )
            );
        });
    ...
    public function prepare_items(): void
    {
        ...
        $filter_order_status = isset($_GET['filter_order_status']) ? sanitize_key(wp_unslash($_GET['filter_order_status'])) : '';
        ...
        $orderby = isset($_GET['orderby']) ? sanitize_key(wp_unslash($_GET['orderby'])) : 'delivery_date';
        // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only order parameter
        $order = isset($_GET['order']) && strtoupper(wp_unslash($_GET['order'])) === 'DESC' ? 'DESC' : 'ASC';
    ...
    public function column_delivery_date(\WC_Order $order): string
    {
        ...
        try {
            ...
            if ($date < $today) {
                $class = 'wct-date-past';
                $badge = '<span class="wct-date-badge past">' . esc_html__('Past', 'checkout-toolkit-for-woo') . '</span>';
            } elseif ($date->format('Y-m-d') === $today->format('Y-m-d')) {
                $class = 'wct-date-today';
                $badge = '<span class="wct-date-badge today">' . esc_html__('Today', 'checkout-toolkit-for-woo') . '</span>';
            } elseif ($date->format('Y-m-d') === $tomorrow->format('Y-m-d')) {
                $class = 'wct-date-tomorrow';
                $badge = '<span class="wct-date-badge tomorrow">' . esc_html__('Tomorrow', 'checkout-toolkit-for-woo') . '</span>';
            }
    ...
    public function column_actions(\WC_Order $order): string
    {
        ...
        $actions['view'] = sprintf(
            ...
            esc_attr__('View Order', 'checkout-toolkit-for-woo'),
            esc_html__('View', 'checkout-toolkit-for-woo')
        );
    ...
    public function extra_tablenav($which): void
    {
        ...
        echo '<option value="">' . esc_html__('All dates', 'checkout-toolkit-for-woo') . '</option>';
        echo '<option value="today"' . selected($filter_date, 'today', false) . '>' . esc_html__('Today', 'checkout-toolkit-for-woo') . '</option>';
        echo '<option value="tomorrow"' . selected($filter_date, 'tomorrow', false) . '>' . esc_html__('Tomorrow', 'checkout-toolkit-for-woo') . '</option>';
        echo '<option value="this_week"' . selected($filter_date, 'this_week', false) . '>' . esc_html__('This week', 'checkout-toolkit-for-woo') . '</option>';
        echo '<option value="this_month"' . selected($filter_date, 'this_month', false) . '>' . esc_html__('This month', 'checkout-toolkit-for-woo') . '</option>';
        echo '</select>';

        // Delivery status filter
        echo '<select name="filter_status" id="filter_status">';
        echo '<option value="">' . esc_html__('All statuses', 'checkout-toolkit-for-woo') . '</option>';
        foreach (DeliveryStatus::get_statuses() as $key => $label) {
        ...
        echo '<option value="">' . esc_html__('All order statuses', 'checkout-toolkit-for-woo') . '</option>';
        ...
        submit_button(__('Filter', 'checkout-toolkit-for-woo'), '', 'filter_action', false);
    ...
    {
        esc_html_e('No deliveries found.', 'checkout-toolkit-for-woo');
    }
}
