...
class DeliveryManager
{
    ...
    public function add_menu_page(): void
    {
        add_submenu_page(
            'woocommerce',
            __('Deliveries', 'checkout-toolkit-for-woo'),
            __('Deliveries', 'checkout-toolkit-for-woo'),
            'manage_woocommerce',
            'wct-deliveries',
    ...
    public function enqueue_assets(string $hook): void
    {
        ...
        wp_localize_script('wct-delivery-admin', 'wctDelivery', [
            ...
            'i18n' => [
                'confirmStatusChange' => __('Change delivery status?', 'checkout-toolkit-for-woo'),
                'statusUpdated' => __('Delivery status updated', 'checkout-toolkit-for-woo'),
                'error' => __('An error occurred', 'checkout-toolkit-for-woo'),
                'loading' => __('Updating...', 'checkout-toolkit-for-woo'),
            ],
        ]);
    }
        ...
        if (!file_exists($template_path)) {
            ...
            return sprintf(
                ...
                sprintf(
                    ...
                    __('Order number: %s', 'checkout-toolkit-for-woo'),
    ...
    public function ajax_update_status(): void
    {
        ...
        if (!current_user_can('manage_woocommerce')) {
            wp_send_json_error(['message' => __('Permission denied', 'checkout-toolkit-for-woo')]);
        }
        ...
        if (!$order_id || !$status) {
            wp_send_json_error(['message' => __('Invalid request', 'checkout-toolkit-for-woo')]);
        }

        $order = wc_get_order($order_id);

        if (!$order) {
            wp_send_json_error(['message' => __('Order not found', 'checkout-toolkit-for-woo')]);
        }
        ...
        if ($success) {
            wp_send_json_success([
                'message' => __('Delivery status updated', 'checkout-toolkit-for-woo'),
                'status' => $status,
                'badge' => DeliveryStatus::get_badge_html($status),
            ]);
        } else {
            wp_send_json_error(['message' => __('Failed to update status', 'checkout-toolkit-for-woo')]);
        }
    }
    ...
    public function ajax_bulk_update_status(): void
    {
        ...
        wp_send_json_success([
            'message' => sprintf(
                ...
                __('%d orders updated', 'checkout-toolkit-for-woo'),
                $updated
            ),
            'updated' => $updated,
            'failed' => $failed,
    ...
    public function add_orders_column(array $columns): array
    {
        ...
        foreach ($columns as $key => $label) {
            ...
            if ($key === 'order_status') {
                $new_columns['wct_delivery'] = __('Delivery', 'checkout-toolkit-for-woo');
            }
        }
    ...
    public function add_dashboard_widget(): void
    {
        ...
        wp_add_dashboard_widget(
            ...
            __('Upcoming Deliveries', 'checkout-toolkit-for-woo'),
