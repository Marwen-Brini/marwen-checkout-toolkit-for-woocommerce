...
class OrderDisplay
{
    ...
    public function display_in_admin(\WC_Order $order): void
    {
        ...
        if (!empty($delivery_method) && !empty($delivery_method_settings['show_in_admin'])) {
            ...
            echo '<p><strong>' . esc_html($delivery_method_settings['field_label'] ?? __('Fulfillment Method', 'marwen-marwchto-for-woocommerce')) . ':</strong><br>';
        ...
        if (!empty($store_location) && !empty($store_locations_settings['show_in_admin'])) {
            ...
            if ($location) {
                echo '<p><strong>' . esc_html($store_locations_settings['field_label'] ?? __('Pickup Location', 'marwen-marwchto-for-woocommerce')) . ':</strong><br>';
                echo '<strong>' . esc_html($location['name']) . '</strong><br>';
                if (!empty($location['address'])) {
                    echo esc_html($location['address']) . '<br>';
                }
                if (!empty($location['phone'])) {
                    echo esc_html__('Phone:', 'marwen-marwchto-for-woocommerce') . ' ' . esc_html($location['phone']) . '<br>';
                }
                if (!empty($location['hours'])) {
                    echo esc_html__('Hours:', 'marwen-marwchto-for-woocommerce') . ' ' . esc_html($location['hours']);
                }
                echo '</p>';
            }
        }
        ...
        if ((!empty($delivery_instructions_preset) || !empty($delivery_instructions_custom)) && !empty($delivery_instructions_settings['show_in_admin'])) {
            echo '<p><strong>' . esc_html($delivery_instructions_settings['field_label'] ?? __('Delivery Instructions', 'marwen-marwchto-for-woocommerce')) . ':</strong><br>';
        ...
        if (!empty($time_window) && !empty($time_window_settings['show_in_admin'])) {
            ...
            echo '<p><strong>' . esc_html($time_window_settings['field_label'] ?? __('Preferred Time', 'marwen-marwchto-for-woocommerce')) . ':</strong><br>';
        ...
        if (!empty($delivery_date) && !empty($delivery_settings['show_in_admin'])) {
            ...
            echo '<p><strong>' . esc_html($delivery_settings['field_label'] ?? __('Delivery Date', 'marwen-marwchto-for-woocommerce')) . ':</strong><br>';
        ...
        if ($custom_field !== '' && !empty($field_settings['show_in_admin'])) {
            ...
            echo '<p><strong>' . esc_html($field_settings['field_label'] ?? __('Special Instructions', 'marwen-marwchto-for-woocommerce')) . ':</strong><br>';
        ...
        if ($custom_field_2 !== '' && !empty($field_2_settings['show_in_admin'])) {
            ...
            echo '<p><strong>' . esc_html($field_2_settings['field_label'] ?? __('Additional Information', 'marwen-marwchto-for-woocommerce')) . ':</strong><br>';
            echo nl2br(esc_html($output)) . '</p>';
        }
    ...
    public function add_meta_box(): void
    {
        ...
        add_meta_box(
            ...
            __('Checkout Toolkit Details', 'marwen-marwchto-for-woocommerce'),
    ...
    public function render_meta_box(\WP_Post|\WC_Order $post_or_order): void
    {
        ...
        if (!empty($delivery_method)) {
            ...
            echo '<span class="delivery-label">' . esc_html($delivery_method_settings['field_label'] ?? __('Fulfillment Method', 'marwen-marwchto-for-woocommerce')) . '</span>';
        ...
        if (!empty($store_location)) {
            ...
            if ($location) {
                ...
                echo '<span class="delivery-label">' . esc_html($store_locations_settings['field_label'] ?? __('Pickup Location', 'marwen-marwchto-for-woocommerce')) . '</span>';
        ...
        if (!empty($delivery_instructions_preset) || !empty($delivery_instructions_custom)) {
            echo '<div class="delivery-row" style="flex-direction: column; align-items: flex-start;">';
            echo '<span class="delivery-label">' . esc_html($delivery_instructions_settings['field_label'] ?? __('Delivery Instructions', 'marwen-marwchto-for-woocommerce')) . '</span>';
            echo '<span class="delivery-value" style="margin-top: 5px;">';
        ...
        if (!empty($time_window)) {
            $time_label = $this->get_time_slot_label($time_window, $time_window_settings);
            echo '<div class="delivery-row">';
            echo '<span class="delivery-label">' . esc_html($time_window_settings['field_label'] ?? __('Preferred Time', 'marwen-marwchto-for-woocommerce')) . '</span>';
        ...
        if (!empty($delivery_date)) {
            $formatted_date = $this->format_date($delivery_date, $delivery_settings['date_format'] ?? 'F j, Y');
            echo '<div class="delivery-row">';
            echo '<span class="delivery-label">' . esc_html($delivery_settings['field_label'] ?? __('Delivery Date', 'marwen-marwchto-for-woocommerce')) . '</span>';
            ...
            echo '<div class="delivery-row">';
            echo '<span class="delivery-label">' . esc_html__('Status', 'marwen-marwchto-for-woocommerce') . '</span>';
        ...
        if ($custom_field !== '') {
            ...
            echo '<div class="delivery-row" style="flex-direction: column; align-items: flex-start;">';
            echo '<span class="delivery-label">' . esc_html($field_settings['field_label'] ?? __('Special Instructions', 'marwen-marwchto-for-woocommerce')) . '</span>';
        ...
        if ($custom_field_2 !== '') {
            ...
            echo '<div class="delivery-row" style="flex-direction: column; align-items: flex-start;">';
            echo '<span class="delivery-label">' . esc_html($field_2_settings['field_label'] ?? __('Additional Information', 'marwen-marwchto-for-woocommerce')) . '</span>';
            echo '<span class="delivery-value" style="margin-top: 5px;">' . nl2br(esc_html($formatted_value)) . '</span>';
            echo '</div>';
        }
        ...
        foreach (array_slice($history, 0, 5) as $entry) {
            ...
            if ($timestamp) {
                ...
                if ($user_name) {
                    ...
                    echo ' ' . sprintf(esc_html__('by %s', 'marwen-marwchto-for-woocommerce'), esc_html($user_name));
                }
                echo '</span>';
            }
            echo '</li>';
